...
<script>
    export default {
        ...
        methods:{
            ...
            async detectMul(){
                if(this.$store.state.fogDetecting || this.$store.state.fogDetecting){
                    alert("有视频正在检测中！")
                    return
                }

                console.log('进行全部视频检测')
                this.total = this.$store.state.fogList.length
                this.num = 0
            ...
            deleteVideo(i){
                ...
                this.axios({
                    method:'post',
                    url:'/video/delete',
                    data:{'idx':i, 'type':this.$store.state.videoType, 'name': this.files[i].name}
                }).then(res=>{
                    console.log(res)
                }).catch(err=>{
                    console.log(err)
