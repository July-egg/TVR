    ...
    export default {
        ...
        methods:{
            ...
            async detectMul(){
                ...
                this.$store.commit('totalChange', [this.$store.state.fogList.length, this.detectType])
                this.$store.commit('numChange', [0, this.detectType])
            ...
            deleteVideo(i){
                ...
                this.axios({
                    method:'post',
                    url:'/video/delete',
                    data:{'idx':i, 'type':this.$store.state.videoType, 'name':this.$store.state.fogList[i].name}
                }).then(res=>{
                    console.log(res)
                }).catch(err=>{
                    console.log(err)
        ...
        computed:{
            present(){ // 当前视频文件
                return this.$store.state.presentFog
            },
            presentIdx(){ // 当前视频文件的索引号
                return this.$store.state.presentFogIdx
            },
            ...
            files:{
                // 返回当前电视机视频列表
                get(){
                    return this.$store.state.fogList
                },
            },
            videoEmpty(){
                return this.$store.state.presentFog == null
            },
            url() {
            ...
            detecting(){
                return this.$store.state.fogDetecting
            },
            total(){
                return this.$store.state.fogTotal
            },
            num(){
                return this.$store.state.fogNum
            }
        },
        watch:{
        ...
        mounted() {
            if(this.$store.state.fogDetecting){
                this.getProgress()
            }
        }
    }
